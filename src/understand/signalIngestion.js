...
export function normalizeSignals(simulation, graph, timestamp = Date.now()) {
  return graph.map((station) => {
    ...
    return createSignal(station.id, measured, { cycleTime: raw.cycleTime, baselineCycleTime: raw.baseCycleTime ?? raw.cycleTime, queueDepth: raw.queueDepth, qualitySignal: raw.qualitySignal }, timestamp);
  });
}
